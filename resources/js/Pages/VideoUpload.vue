<template>
    <form @submit.prevent="submit">
        <div class="flex items-center justify-center w-full">
            <label class="flex flex-col items-center justify-center w-3/4 h-[32rem] border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500"
            @click.stop.prevent="() => console.log('clicked the container')"
            >
                <div class="flex flex-col items-center justify-center pt-5 pb-6 bg-gray-200 w-3/4 my-4 rounded-lg"
                     @drop.prevent="handleDrop"
                     @dragover.prevent="handleDragOver"
                     @click="() => console.log('clicked inside the file pane')"
                ...
                @click="() => console.log('clicked inside the dropzone file list div...')"
                >
                    <div v-for="(file, index) in files" :key="index" class="py-2 mx-12">
                        <div class="flex items-center justify-between w-full text-sm my-1">
                            <div class="flex items-center gap-2 flex-grow">
                                <img src="data:image/png;base64," alt="File Icon" class="w-8 h-8"/>
                            ...
                            <span @click="removeFile(index)" class="cursor-pointer text-red-500 hover:fill-red-500">
                                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="26" height="26" viewBox="0,0,300,150">
                        ...
                        <div class="w-full bg-gray-200 rounded-full dark:bg-gray-700">
                            <div class="bg-black p-0.5 leading-none rounded-full" style="width: 90%"></div>
                        </div>
...
function updateFileList(newFiles) {
    ...
    files.value = [...files.value.filter(file => !validFiles.some(newFile => newFile.name === file.name)), ...validFiles];
}
